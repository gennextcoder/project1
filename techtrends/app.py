...
def post(post_id):
    ...
    if post is None:
      ...
      app.logger.debug('Article ' + post[2] + ' retrieved!')  
...
def metrics():
    ...
    response = app.response_class(
            response=json.dumps({"db_connection_count":num_db_connections,"post_count":num_posts}),
            status=200,
            mimetype='application/json'
    )
...
@app.route('/about')
def about():
    app.logger.debug('The "About us" page has been retrieved!') 
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            connection.commit()
            connection.close()
            app.logger.debug('Article ' + title + ' has been created!') 
